...
const proveedores = proveedoresIniciales
...
export function GestionPagosProveedores({ onBack, showToast, pagosPendientes, setPagosPendientes }: GestionPagosProps) {
  ...
  const searchPagosPendientes = (q: string) => {
    if (!q) return pagosPendientes
  ...
  const handleRegistrarPago = () => {
    ...
    const nuevoPago: RegistroPago = {
      ...
      medioPago: mediosPago.find((m) => m.id === medioPago)?.nombre || "Efectivo",
                ...
                <SelectContent className="bg-card border-border">
                  {proveedores.map((prov) => (
                    <SelectItem key={prov.id} value={prov.id}>
                      <span className="text-foreground">{prov.nombre}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={medioPago} onValueChange={setMedioPago} className="grid grid-cols-3 gap-3">
                {mediosPago.map((medio) => (
                  <div key={medio.id}>
                    <RadioGroupItem value={medio.id} id={medio.id} className="peer sr-only" />
                    <Label
                      htmlFor={medio.id}
                    ...
                    >
                      <medio.icon className="w-6 h-6 mb-2" />
                      <span className="text-sm text-center">{medio.nombre}</span>
                    </Label>
                  </div>
                ))}
              </RadioGroup>
...
// Icon placeholder since Plus is not imported
